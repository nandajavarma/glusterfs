                                      ...
                                      dict_t *rsp_dict)
{
        ...
        for (i = 1; i <= volcount; i++) {
                ...
                list_for_each_entry (brickinfo, &volinfo->bricks, brick_list) {
                        ...
                        if (ret) {
                                gf_msg (this->name, GF_LOG_ERROR, 0,
                                        GD_MSG_BRICK_SET_INFO_FAIL, "failed to "
                             ...
                             gf_boolean_t force)
{
        ...
        if (ret) {
                gf_msg (this->name, GF_LOG_WARNING, 0,
                        GD_MSG_VOLINFO_REMOVE_FAIL, "Failed to remove volinfo "
                       "%s ", snap_vol->volname);
                save_ret = ret;
                if (!force)
                        goto out;
                                    ...
                                    int64_t volcount, int32_t brick_count)
{
        ...
        if (add_missed_snap) {
                ...
                if (ret) {
                        gf_msg (this->name, GF_LOG_ERROR, 0,
                                GD_MSG_MISSEDSNAP_INFO_SET_FAIL, "Failed to add missed"
                      ...
                      dict_t *dict, dict_t *rsp_dict, int64_t volcount)
{
        ...
        if (ret) {
                gf_msg (this->name, GF_LOG_ERROR, 0,
                        GD_MSG_VOL_OP_FAILED, "Failed to copy geo-rep "
        ...
        if (ret) {
                gf_msg (this->name, GF_LOG_ERROR, 0,
                        GD_MSG_SNAP_LIST_SET_FAIL, "could not add the snap "
...
glusterd_take_brick_snapshot_task (void *opaque)
{
        ...
        if (ret) {
                gf_msg (THIS->name, GF_LOG_ERROR, 0,
                        GD_MSG_SNAPSHOT_OP_SUCCESS, "Failed to "
                                 ...
                                 dict_t *rsp_dict)
{
        ...
        if (ret) {
                gf_msg (this->name, GF_LOG_ERROR, 0,
                        GD_MSG_SNAPSHOT_OP_SUCCESS, "Failed to take backend "
...
glusterd_snapshot_revert_partial_restored_vol (glusterd_volinfo_t *volinfo)
{
        ...
        if (ret) {
                gf_msg (this->name, GF_LOG_ERROR, 0,
                        GD_MSG_VOLINFO_REMOVE_FAIL, "Failed to delete volinfo");
                goto out;
        }
...
glusterd_snapshot_revert_restore_from_snap (glusterd_snap_t *snap)
{
        ...
        if (ret) {
                gf_msg (this->name, GF_LOG_ERROR, 0,
                        GD_MSG_SNAP_RESTORE_REVERT_FAIL, "Failed to revert snapshot "
                                  ...
                                  char **op_errstr, dict_t *rsp_dict)
{
        ...
        if (0 == op_ret) {
                ...
                if (ret) {
                        gf_msg (this->name, GF_LOG_ERROR, 0,
                                GD_MSG_SNAP_RESTORE_REVERT_FAIL, "Failed to revert "
                                "restore operation for %s volume", volname);
                        goto out;
                }
